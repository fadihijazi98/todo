...
@section('content')
    <div class="">
        <div class="flex flex-col-reverse sm:flex-row flex-row justify-center py-12 px-4 sm:px-6 lg:px-8 gap-3">
            <div class="">
                <form action="{{route('board.update', $task)}}" method="post" class="mt-8 space-y-6">
                    @csrf
                    @method('put')
                    ...
                    <div class="rounded-md shadow-sm -space-y-px">
                        <div class="">
                            ...
                            <input id="name" name="name" type="text" autocomplete="name" required value="{{$task->name}}"
                    ...
                    <div class="">
                        ...
                        <select id="color" name="color_id" autocomplete="color"
                            ...
                            @foreach($colors as $color)
                                <option value="{{$color->id}}" {{$color->id==$task->color_id?'selected':''}}>
                                    {{ $color->name }}
                                </option>
                            @endforeach
                        </select>
